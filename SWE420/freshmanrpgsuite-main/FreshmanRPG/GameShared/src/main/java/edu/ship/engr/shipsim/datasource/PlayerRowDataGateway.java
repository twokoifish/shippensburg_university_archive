...
public class PlayerRowDataGateway
{
    ...
    public void updateCrewID (int crewID) throws DatabaseException
    {
        this.connection = DatabaseManager.getSingleton().getConnection();

        try (PreparedStatement stmt = connection.prepareStatement(
                "UPDATE Players SET crew = ? WHERE playerID = ?"))
        {
            stmt.setInt(1, crewID);
            stmt.setInt(2, playerID);
            stmt.executeUpdate();
            this.crew = Crew.getCrewForID(crewID);
        }
        catch (SQLException e)
        {
            throw new DatabaseException("Couldn't update crew for player with ID " + playerID, e);
        }
    }
}
