...
public class QuestStateTableDataGatewayTest
{
    ...
    public void canChangeExistingState() throws DatabaseException
    {
        ...
        ArrayList<QuestStateRecordDTO> actual =
                gateway.getQuestStates(playerID);
        for (QuestStateRecordDTO qsRec : actual)
        {
            if ((qsRec.getPlayerID() == playerID) &&
                    (qsRec.getQuestID() == questID))
            {
                ...
                assertEquals(curr.toString(),
                        qsRec.getDateCompleted().toString());
            }
        }
    }
    ...
    public void canInsertARecord() throws DatabaseException
    {
        ...
        ArrayList<QuestStateRecordDTO> actual =
                gateway.getQuestStates(
                        QuestStatesForTest.PLAYER1_QUEST1.getPlayerID());
        assertEquals(10, actual.size());
        assertTrue(actual.contains(
                new QuestStateRecordDTO(
                        QuestStatesForTest.PLAYER1_QUEST1.getPlayerID(),
                        4, QuestStateEnum.TRIGGERED, true, null)));
        ...
        for (int i = 0; i < 5; i++)
        {
                    ...
                    QuestStatesForTest.PLAYER1_QUEST2.getQuestID())
            {
                ...
                assertEquals(expected.getState(),
                        record.getState(),
                        "Invalid state for quest id " + record.getQuestID());
            }
        }
    }
